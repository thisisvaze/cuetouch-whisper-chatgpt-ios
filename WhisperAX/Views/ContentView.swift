...
struct ContentView: View {
    ...
    // Provide me short pointers on the next piece of conversation. Give me 2 pointers (max 6 words each).
    // """
    ...
    func resetState() {
        ...
        confirmedSegments = []
        unconfirmedSegments = []
        apiResponseSummary = "Start recording and get suggestions" 
    }
...
//                .padding()
        ...
        VStack {
                ...
                //modelSelectorView
                transcriptionView
                    .frame(maxWidth: .infinity, alignment: .top) // Align to top
            #elseif os(macOS)
                ...
                .padding()
            #endif
            Spacer() // Pushes everything above to the top and button to the bottom
            controlsView
                .frame(maxWidth: .infinity, alignment: .top) // Align to top
            Button(action: {
                Task {
                ...
                Text("Get suggestions")
                    .frame(minWidth: 200, minHeight: 60) // Set the minimum width and height
                    .font(.title2) // Set the font size
                    .foregroundColor(.white) // Set the text color
                    .padding(.vertical) // Add vertical padding
            }
            .frame(maxWidth: .infinity) // Ensure the button expands to fill the width
            .background(isRecording ? Color.blue : Color.gray) // Set the background color
        ...
        .onAppear {
            ...
            #endif
            fetchModels()
            // Set the selected model to "tiny"
            selectedModel = "tiny"
            // Load the "tiny" model
            loadModel(selectedModel)

            // Preselect the "Stream" category
            if let streamCategory = menu.first(where: { $0.name == "Stream" }) {
                selectedCategoryId = streamCategory.id
            }
        }
    }
    ...
    var controlsView: some View {
        VStack {
            ...
            if let selectedCategoryId, let _ = menu.first(where: { $0.id == selectedCategoryId }) {
                //switch item.name {
//                case "Transcribe":
//                    VStack {
//                        HStack {
                ...
                VStack {
                    Button {
                            ...
                            Image(systemName: !isRecording ? "record.circle" : "stop.circle.fill")
                                ...
                                .frame(width: 70, height: 70)
                                .padding()
                                .foregroundColor(modelState != .loaded ? .gray : .blue)
                        }
                        .contentTransition(.symbolEffect(.replace))
                        .buttonStyle(BorderlessButtonStyle())
                        .disabled(modelState != .loaded)
    ...
    do {
        ...
        if let httpResponse = response as? HTTPURLResponse, httpResponse.statusCode == 200 {
            if let json = try JSONSerialization.jsonObject(with: data) as? [String: Any],
               let answers = json["choices"] as? [[String: Any]],
               ...
               let content = message["content"] as? String {
                ...
                //show pointers --- WhisperWings ----
//                DispatchQueue.main.async {
//                    self.apiResponseSummary = content
//                }
            ...
            // Handle the error
            print("Error fetching the suggestion: \(response)")
        }
    } catch {
        // Handle the error
